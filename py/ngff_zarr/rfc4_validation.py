...
def validate_rfc4_orientation(axes: list[dict[str, Any]]) -> None:
    ...
    # The bundled artifact reports nothing: its root is an open object with
    # every definition parked in ``$defs`` and no ``$ref`` reaching them, so any
    # JSON object satisfies it (upstream ome/ngff#585 and #586). The RFC 4 rules
    # that fire are the hand-written Python above, which
    # ``test_the_bundled_rfc4_schema_checks_nothing`` pins. The call stays: it
    # starts reporting the moment upstream gives the artifact an entry point.
