...
read and written at versions 0.4, 0.5, and 0.6 (and the opt-in development
version `0.9.dev1`):
...
- **Zipped plates**: Plates at the Zarr v3 versions (0.5, 0.6, and the opt-in
  `0.9.dev1`) can be written to and read from RFC-9 `.ozx` archives
...
- **Any Zarr v3 version**: OME-Zarr 0.5 and 0.6 (and the opt-in `0.9.dev1`) can be zipped; 0.4 (Zarr v2) cannot
