...
def test_rfc3_real_reference_data():
    ...
    never downloads or requires it. Asserts the driver produces a classified
    report for every manifest case; the pass/fail verdict itself is the
    standalone driver's exit code (see ``rfc3_conformance.py``).
    """
    import sys
    from pathlib import Path

    sys.path.insert(0, str(Path(__file__).resolve().parent))
    from rfc3_conformance import MANIFEST, run_data_dir

    reports = run_data_dir(Path(os.environ["RFC3_DATA_DIR"]))
    assert len(reports) == len(MANIFEST)
    for report in reports:
        assert report["status"] in {"pass", "fail", "skip"}
