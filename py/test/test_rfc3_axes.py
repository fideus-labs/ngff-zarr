...
needs_zarr_v3 = pytest.mark.skipif(
    packaging_version.parse(zarr.__version__) < packaging_version.parse("3.0.0b1"),
...
@pytest.mark.skipif(
    not os.environ.get("RFC3_DATA_DIR"),
    reason="set RFC3_DATA_DIR to the generated clbarnes/ome-zarr-rfc3-data/data "
    "directory to run the real-reference-data integration (opt-in, no network)",
)
def test_rfc3_real_reference_data():
    """Drive the RFC-3 conformance driver against the real reference datasets.

    Opt-in only (``RFC3_DATA_DIR``): the official RFC-3 data is generated by
    ``clbarnes/ome-zarr-rfc3-data`` and is not checked in, so the default suite
    never downloads or requires it. The only tolerated failure is the
    undefined ``0.5+rfc3`` version tag; a copy retagged ``0.9.dev1`` must then
    fully conform.
    """
    import sys
    ...
    from rfc3_conformance import MANIFEST, VERSION_STRING, run_data_dir

    reports = run_data_dir(Path(os.environ["RFC3_DATA_DIR"]))
    assert len(reports) == len(MANIFEST)
    for report in reports:
        if report["status"] == "skip":
            continue
        if report["status"] == "fail":
            assert report["classification"] == VERSION_STRING, report
            assert report["version_normalized"]["status"] == "pass", report
        else:
            assert report["status"] == "pass", report


@pytest.mark.parametrize("version", ["0.5", "0.6"])
def test_rfc3_upgrade_in_place_is_refused_below_v09dev1(tmp_path, version):
    """upgrade_ome_zarr refuses an RFC-3 axis model and leaves the store intact.

    This path bypasses the writer's normal metadata preparation, so it carries
    its own gate call.
    """
    from ngff_zarr import upgrade_ome_zarr

    axes = [{"name": n, "type": "space"} for n in "abcdef"]
    root = _write_v04(tmp_path / f"upgrade-{version}.ome.zarr", axes, (2,) * 6)
    before = (tmp_path / f"upgrade-{version}.ome.zarr" / ".zattrs").read_text()

    with pytest.raises(ValueError, match="Cannot write OME-Zarr"):
        upgrade_ome_zarr(root, version=version)

    after = (tmp_path / f"upgrade-{version}.ome.zarr" / ".zattrs").read_text()
    assert after == before, "a refused upgrade must not modify the source store"
