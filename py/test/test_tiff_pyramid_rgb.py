...
def test_pyramidal_rgb_tiff_channel_consistency():
    ...
    assert isinstance(result, Multiscales), "Should return Multiscales for pyramidal TIFF"
    
    # Check that we have 3 pyramid levels
    assert len(result.images) == 3, f"Expected 3 pyramid levels, got {len(result.images)}"
...
def test_pyramidal_grayscale_tiff():
    ...
    for i, img in enumerate(result.images):
        assert img.dims == ("y", "x"), f"Level {i} should have dims ('y', 'x'), got {img.dims}"
